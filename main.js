...
function create() {
    ...
    this.add.image(600, 400, 'background').setScale(2); // Adjust the scale to fit the game dimensions
   ...
   // Create the ground as a static physics object
    ...
    for (let i = 0; i < 2; i++) {
        let groundSegment = ground.create(200 + (i * 400), 848, 'ground').setScale(0.4).refreshBody(); // Adjust the initial position
    ...
    for (let i = 0; i < 2; i++) {
        let floatingSegment = floating_ground.create(610 + (i * 1200), 528, 'ground').setScale(Phaser.Math.FloatBetween(0.09, 0.07)).refreshBody(); // Adjust the initial position
    ...
    dog = this.physics.add.sprite(100, 1280, 'dog');
...
function update(time, delta) {
    ...
    floating_ground.children.iterate(function (child) {
        ...
        let randomOffset = Phaser.Math.Between(1900, 525);
        if (child.x < -randomOffset) {
            child.x = Phaser.Math.Between(3055, 1500);
            child.y= Phaser.Math.Between(528, 300);
...
function spawnHoover() {
    const x = 1230; // Spawn hoover on the right side of the screen
    const y = 700; // Random Y position between 500 to 700
